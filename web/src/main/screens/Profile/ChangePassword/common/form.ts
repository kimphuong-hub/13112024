...
export const validationSchema = (t: TFunction<'translation', undefined>) =>
  yup.object({
    ...
    reNewPassword: yup
      ...
      .oneOf([yup.ref('newPassword')], t('app.profile.change-password.condition.match')),
    previousPassword: yup.string().required(t('app.profile.master-data.input.previous-password.required'))
  });
